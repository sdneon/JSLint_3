...
(function(mod) {
  ...
  function endOperation_finish(op) {
    ...
    if (hidden) for (var i = 0; i < hidden.length; ++i)
      //SD: pass in emitter itself, o.w. event handler gets no clue at all!!
      //if (!hidden[i].lines.length) signal(hidden[i], "hide");
      if (!hidden[i].lines.length) signal(hidden[i], "hide", hidden[i]);
    if (unhidden) for (var i = 0; i < unhidden.length; ++i)
      //if (unhidden[i].lines.length) signal(unhidden[i], "unhide");
      if (unhidden[i].lines.length) signal(unhidden[i], "unhide", unhidden[i]);
  ...
  function updateLine(line, text, markedSpans, estimateHeight) {
    ...
    if (estHeight != line.height) updateLineHeight(line, estHeight);
    //SD: Emit new additional "moved" signal to inform markers!
    if (markedSpans)
    {
        var cnt = markedSpans.length, i, mark;
        for (i = 0; i < cnt; ++i)
        {
            mark = markedSpans[i].marker;
            if (mark)
            {
                signalLater(mark, "moved", mark);
            }
        }
    }
  }
  ...
  //SD: Export 'CodeMirror' globally to make it visible to CM addons when loaded by Dojo.
